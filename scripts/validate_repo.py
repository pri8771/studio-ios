...
VALID_GENERAL = {"draft", "in-review", "approved", "active", "blocked", "needs-update", "deprecated", "archived"}
...
def validate_yaml_like(path: Path, text: str, errors: list[str]) -> None:
    ...
    if record_type and path.parts[-2] not in {"registry"}:
...
def main() -> int:
        ...
        if path.suffix in {".yaml", ".yml"}:
            ...
            if record_id:
                if record_id in seen_ids:
                    errors.append(f"{path}: duplicate id '{record_id}' also in {seen_ids[record_id]}")
                else:
    ...
    print(f"Checked {checked} files and {len(seen_ids)} stable IDs.")
    if errors:
